!!==============================================================================
!! Implement of the second version of equations of motion(EOMs) of paper we
!! comment on.
!!
!! Note: in this version, Huo et. al. eventually came up correct EOMs on 
!! Stratonovich-Weyl(SW) space with a correct definition of coherent state (May 27). 
!! Notice our work 10.1002/wcms.1619 has long been published. (Submit on Feb 5, 
!! Received for proof on Mar 15, feedbacked on Apr 8, and published on May 13).
!!
!! Yet saying Huo et. al. lacked respect to our work aside, they tried to silently
!! correct their previous errors and pretend they had worked out the correct
!! dynamics on SW space for the first time.
!!
!! We have ample grounded reasons to fight back the allegation and even rumors 
!! they have made upon us, and all things provided here could prove our innocence.
!!
!! Additionally, Huo et. al. also directly copied the global phase to this version,
!! which just clumsily imitated 10.1002/wcms.1619. And they totally
!! misunderstood the delicate role of global phase other than a constant value.
!! It is enough to show that Huo et. al. are not so clear about the correct 
!! relationship between Meyer-Miller and SW variables. The complete
!! information should involve at least (2F-1) variables other than (2F-2)!
!!
!!==============================================================================
...
!! Asym-type (alpha labeled): like sigma_x, saved L-block
!! Bsym-type (beta labeled): like sigma_y, saved U-block
...
!!==============================================================================
!! definition of coherent state. (version LATE in May 27)
...
implicit none
    ...
    do n=2, F-1
        c(n) = cos(theta(n)/2)
        do l=1,n-1
            c(n) = c(n) * sin(theta(l)/2) * exp((0.0d0,1.0d0)*varphi(l) )
        enddo
    enddo
    c(F) = (1.0d0,0.0d0)
    do l=1,F-1
        c(F) = c(F) * sin(theta(l)/2) * exp((0.0d0,1.0d0)*varphi(l) )
    enddo
end subroutine
...
implicit none
    ...
    global_phase = atan2(aimag(c(1)), real(c(1)))
    gp0 = global_phase ! save "global phase" at zero time

    c(:) = c(:) / exp((0.0d0,1.0d0)*global_phase) ! erase global_phase

    !! conversion c(:) to theta & varphi
    !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if(F.ne.3) stop "this conversion is specific in F=3 case"
    theta(1) = 2*acos(real(c(1)))
    varphi(1) = atan2(aimag(c(2)), real(c(2)))
    theta(2) = 2*acos(real(c(2)/sin(theta(1)/2)/exp((0.0d0,1.0d0)*varphi(1))))
    c(3) = c(3) / sin(theta(1)/2) / sin(theta(2)/2)
    varphi(2) = atan2(aimag(c(3)), real(c(3))) - varphi(1)
    ...
    do i=0,Ntime
        if(mod(i,Ns) == 0) then
            c_corr = c * exp((0.0d0, 1.0d0)*global_phase) !< global_phase = gp0
